"""Projection-based steering with L2 re-normalization: amplify the component
of each memory embedding that aligns with dream-director theme vectors, then
re-normalize so that steering only affects direction, not magnitude.
...
Math per theme t and memory embedding m:
    ...
    output   = steered / ||steered||     # re-normalize to unit length
"""
...
class ProjectionNormalizeSteerer:
    """Amplifies theme-aligned components of memory embeddings, then
    re-normalizes to unit length so only direction is affected."""
...
def add_args(parser: argparse.ArgumentParser) -> None:
    """Register projection-normalize steerer CLI arguments."""
    group = parser.add_argument_group("projection-normalize steerer")
    group.add_argument(
        "--themes-file",
        default=None,
...
def create(args: argparse.Namespace, embedder) -> ProjectionNormalizeSteerer:
    """Build a ProjectionNormalizeSteerer from parsed CLI args."""
    ...
    if args.themes_file:
        ...
        raise ValueError(
            "Projection-normalize steerer requires --themes-file or --themes"
        )
    ...
    return ProjectionNormalizeSteerer(
